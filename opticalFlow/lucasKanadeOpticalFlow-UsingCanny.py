...
if savevid:
    ...
    savepath = vidpath.split('.')[-2] + '_LK_FLOW_Canny' + '.mp4'
    savepathCanny = vidpath.split('.')[-2] + '_LK_FLOW_Canny_Edges' +  '.mp4'
    print(f"Saving Output video to: {savepath}")
    print(f"Saving Canny Output video 1 to: {savepathCanny}")
    ...
    fourccCanny = cv.VideoWriter_fourcc(*'mp4v')
    videoOutCanny = cv.VideoWriter(savepathCanny, fourccCanny, fps, (width, height))
